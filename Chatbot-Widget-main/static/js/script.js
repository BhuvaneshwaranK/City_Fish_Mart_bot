...
function showBotTyping() {
  const botTyping =
    '<img class="botAvatar" id="botAvatar" src="./static/img/fish_icon.jpg"/><div class="botTyping"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div>';
...
function setBotResponse(response) {
  ...
  setTimeout(() => {
    ...
    if (response.length < 1) {
      ...
      const BotResponse = `<img class="botAvatar" src="./static/img/fish_icon.jpg"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
      ...
      for (let i = 0; i < response.length; i += 1) {
        ...
        if (Object.hasOwnProperty.call(response[i], "text")) {
          if (response[i].text != null) {
            const BotResponse = `<img class="botAvatar" src="./static/img/fish_icon.jpg"/><p class="botMsg">${response[i].text.replace(/(?:\r\n|\r|\n)/g, '<br>')}</p><div class="clearfix"></div>`;
            $(BotResponse).appendTo(".chats").hide().fadeIn(1000);
          }
        }
...
//=============Initiate chatbot with sample greet message
function greetMessage(res) {
  setTimeout(function() { 
      setBotResponse([res])
  }, 1000);
}
  ...
  $.ajax({
    ...
    success(botResponse, status) {
      ...
      if (message.toLowerCase() === "/restart") {
        ...
        res = [{"recipient_id":sender_id,"text":"Hello, 👋 I'm Nemo, your CityFishMart AI chat assistant."},
              {"recipient_id":sender_id,"text":"I can help you ordering fish products from your favourite CityFishMart."}]
            var resLength = res.length;
            // Loop through responses
            for (i = 0; i < resLength; i++) {
                // setBotResponse([res[i]])
                greetMessage(res[i])
            }
            scrollToBottomOfResults();
        $("#userInput").prop("disabled", false);
